...
speedmat_old_deprecated <- function(sf,
                                    mode = "CDE", # "CE"/"DE"/"CDE"
  ...
  if (!grepl("E", mode)) stop("Please use the methods for standard spatial weight matrices for non-entropy matrices")
  ...
  sf_ent <- sf %>%
    ...
    mutate_at(.vars = vars(input_vars), .funs = list(~ as.vector(scale(.)))) %>%
    mutate_at(.vars = vars(everything()), .funs = list(~ . + abs(min(.)))) %>%
    as.matrix %>%
    philentropy::JSD(.)
  ...
  if (grepl("D", mode)) {
    ...
    if (grepl("^C", mode)) {
      if (
        !any(st_is(sfp, 'POLYGON'), st_is(sfp, 'MULTIPOLYGON'), st_is(sfp, "POLYGON Z"), st_is(sfp, "MULTIPOLYGON Z"))
      ) {
        sf_touch_ <- sf_touch
